...
class PlayState(BaseState):
    ...
    def update(self, dt: float) -> None:
        ...
        for ball in self.balls:
            ...
            # Check collision with sticky paddle
            if ball.collides(self.paddle) and self.paddle.is_sticky:
                dif_left = ball.x - self.paddle.x
                dif_right = self.paddle.x + self.paddle.width - ball.x
                
                ball.vx = self.paddle.vx
                ball.vy = 0
                ball.x = max(dif_left, min(ball.x,settings.VIRTUAL_WIDTH - dif_right))
                settings.SOUNDS["paddle_hit"].stop()
                settings.SOUNDS["paddle_hit"].play()
                settings.SOUNDS["paddle_hit"].stop()
                #ball.rebound(self.paddle)
            ...
            """ if ball.is_explosive:
                brick_left = self.brickset.get_brick(brick.x // 32 - 1, brick.y // 16)
                brick_right = self.brickset.get_brick(brick.x // 32 + 1, brick.y // 16)
                brick_top = self.brickset.get_brick(brick.x // 32, brick.y // 16 + 1)
                brick_bot = self.brickset.get_brick(brick.x // 32, brick.y // 16 - 1)
                brick.hit()
                brick_top.hit()
                brick_bot.hit()
                brick_right.hit()
