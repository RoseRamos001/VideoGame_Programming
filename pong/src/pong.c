...
void update_pong(struct Pong* pong, double dt)
{
    ...
    if (pong->ball.vx > 0 && pong->ball.x > TABLE_WIDTH / 2)
        {
            pong->x_distance = pong->player2.x - (pong->ball.x + pong->ball.width);
            pong->t = pong->x_distance / pong->ball.vx;
            pong->final_y = pong->ball.vy * pong->t + pong->ball.y + pong->ball.height / 2;
            if (pong->final_y > pong->player2.y + pong->player2.height){
                pong->player2.vy = PADDLE_SPEED;
            }
            else if (pong->final_y < pong->player2.y)
            {
                pong->player2.vy = -PADDLE_SPEED;
            }
            else if (pong->final_y >= pong->player2.y && pong->final_y <= (pong->player2.y + pong->player2.height))
            {
                pong->player2.vy = 0;
            }       
        }
